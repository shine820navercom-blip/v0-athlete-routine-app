...
import { ArrowLeft, Trophy, TrendingUp, Plus, Trash2 } from "lucide-react"
...
export default function GameResultsPage() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("gameUpdated", handleGameUpdate)
  }, [])
  ...
  const loadGames = () => {
    ...
    // Calculate correlation between routine completion and win rate
    setCorrelationData([
  ...
  const handleDeleteGame = (gameId: string) => {
    ...
    const newStats = { ...stats }
    if (gameToDelete.result === "win") newStats.wins = Math.max(0, newStats.wins - 1)
    else if (gameToDelete.result === "loss") newStats.losses = Math.max(0, newStats.losses - 1)
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <Dialog open={isAddingGame} onOpenChange={setIsAddingGame}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>새 경기 기록 추가</DialogTitle>
              </DialogHeader>
          ...
          <div className="space-y-3">
            {games.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                <p>아직 기록된 경기가 없습니다</p>
                <Button className="mt-4 bg-transparent" variant="outline" onClick={() => setIsAddingGame(true)}>
                  경기 추가하기
                </Button>
              </div>
            ) : (
